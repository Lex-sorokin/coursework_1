import json
import os
from datetime import datetime
from typing import Any

import numpy as np
import pandas as pd
import requests
from dotenv import load_dotenv
from pandas import DataFrame

from loger import module_logger

logger = module_logger(__name__)

load_dotenv()


def hello_greeting(date_time: str) -> str:
    """Функция, которая приветствует пользователя в зависимости от времени суток"""
    logger.info("Функция hello_greeting запущена")
    now = datetime.strptime(date_time, "%Y-%m-%d %H:%M:%S")
    result = ""
    if 0 < now.hour < 4 or 22 < now.hour < 24:
        result = "Доброй ночи"
    elif 4 < now.hour < 10:
        result = "Доброе утро"
    elif 10 < now.hour < 16:
        result = "Добрый день"
    elif 16 < now.hour < 22:
        result = "Добрый вечер"
    logger.info("Функция успешно завершила работы")
    return result


def read_excel(file_path: str) -> DataFrame:
    """Функция, которая считывает финансовые операции из XLSX-файла."""
    logger.info("Функция read_excel запущена")
    df = pd.read_excel(file_path)
    logger.info("Функция успешно завершила работы")
    return df.replace({np.nan: None})


def extract_cards(df: DataFrame) -> list[dict]:
    """Функция, которая обрабатывает данные из DataFrame и возвращает список словарей"""
    logger.info("Функция extract_cards запущена")
    transaction_list: DataFrame = df.groupby("Номер карты").agg({"Сумма операции с округлением": "sum"})
    list_cards = [
        {
            "last_digits": str(group)[-4:],
            "total_spent": round(row["Сумма операции с округлением"], 2),
            "cashback": round(row["Сумма операции с округлением"] / 100, 2),
        }
        for group, row in transaction_list.iterrows()
    ]
    logger.info("Функция успешно завершила работы")
    return list_cards


def sort_by_sum(df: DataFrame) -> list[dict]:
    """Функция, которая возвращает Топ-5 транзакций по сумме платежа."""
    logger.info("Функция sort_by_sum запущена")
    return_data = []
    ss = ["Дата платежа", "Сумма платежа", "Категория", "Описание"]
    result = df[ss][(df["Статус"] == "OK")].sort_values("Сумма платежа", ascending=True).head()
    for i, row in result.iterrows():
        data = {
            "date": row["Дата платежа"],
            "amount": row["Сумма платежа"],
            "category": row["Категория"],
            "description": row["Описание"],
        }
        return_data.append(data)
    logger.info("Функция успешно завершила работы")
    return return_data


def user_settings(path: str) -> dict[Any, Any]:
    """Функция, которая открывает .JSON файл с параметрами"""
    logger.info("Функция sort_by_sum запущена")
    with open(path, encoding="utf8") as f:
        read_json = json.load(f)
    logger.info("Функция успешно завершила работы")
    return dict(read_json)


def get_currency_rates(settings_user: dict) -> list[dict]:
    """Функция, которая запрашивает курс валют по API."""
    logger.info("Функция get_currency_rates запущена")
    data = []
    headers = {"apikey": os.getenv("API_KEY")}
    url = "https://api.apilayer.com/exchangerates_data/2025-07-12"
    for currency in settings_user["user_currencies"]:
        payload = {"base": currency, "symbols": "RUB"}

        response = requests.get(url, headers=headers, params=payload)
        json_response = response.json()
        data.append({"currency": currency, "rate": json_response["rates"]["RUB"]})
    logger.info("Функция успешно завершила работы")
    return data


def get_stock_prices(settings_user: dict) -> list[dict]:
    """Функция, которая запрашивает курс акций по API."""
    logger.info("Функция get_stock_prices запущена")
    data = []

    url = "https://www.alphavantage.co/query"
    for stock in settings_user["user_stocks"]:
        payload = {
            "function": "GLOBAL_QUOTE",
            "symbol": stock,
            "apikey": os.getenv("API_KEY_ALPHAVANTAGE"),
        }

        response = requests.get(url, params=payload)
        json_response = response.json()
        data.append({"stock": stock, "price": float(json_response["Global Quote"]["05. price"])})
    logger.info("Функция успешно завершила работы")
    return data
